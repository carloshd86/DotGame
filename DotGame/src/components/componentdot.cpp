...
void CDot::CheckInteractAction(const Event& event)
{
	...
	if (eMouseClick)
	{
		if (IsPositionOverDot(Vec2(gMouseX, gMouseY)))
		{
			...
			{
				if (Game::DotType::Green == mType) { if (g_pSoundManager) { g_pSoundManager->PlaySound(gHitSoundId);  } g_pGame->IncreaseScore(mOwner);               }
				else                               { if (g_pSoundManager) { g_pSoundManager->PlaySound(gFailSoundId); } g_pGame->SetGameOver(Game::GameResult::Fail); }
			}
			else if (EventMouseClick::EMouseButton::Right == eMouseClick->GetMouseButton())
			{
				if (Game::DotType::Red == mType) { if (g_pSoundManager) { g_pSoundManager->PlaySound(gHitSoundId);  } g_pGame->IncreaseScore(mOwner);			    }
				else                             { if (g_pSoundManager) { g_pSoundManager->PlaySound(gFailSoundId); } g_pGame->SetGameOver(Game::GameResult::Fail); }
			}
		}
	}
